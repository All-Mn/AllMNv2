...
const char * const BITCOIN_CONF_FILENAME = "allmn.conf";
const char * const BITCOIN_PID_FILENAME = "allmnd.pid";
 ...
 * by well-meaning allmn adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "allmn" is a composite category enabling all ALLMN-related debug output
            if(ptrCategory->count(string("allmn"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ALLMNCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ALLMNCore
    // Mac: ~/Library/Application Support/ALLMNCore
    // Unix: ~/.allmncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ALLMNCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ALLMNCore";
#else
    // Unix
    return pathRet / ".allmncore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty allmn.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override allmn.conf
