...
static void MixColumns(AES_state* s, int inv) {
     ...
     * coefficients of a 3rd degree polynoallmnl over GF(2^8) and multiplies them
     * by the fixed polynoallmnl a(x) = {03}x^3 + {01}x^2 + {01}x + {02}, modulo
     * x^4 + {01}.
     *
...
/* Multiply the cells in s by x, as polynoallmnls over GF(2) mod x^8 + x^4 + x^3 + x + 1 */
